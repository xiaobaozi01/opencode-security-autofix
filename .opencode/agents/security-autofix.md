...
3. 修改前让 `fix-validator` 执行 `task_preflight`：确认任务起始 `HEAD`、工作区状态、Scanner 报告或人工代码 baseline、验证命令和结果目录忽略状态。保存一份任务级证据，不得由主 Agent 重写。
...
9. 全部可用 Patch 集成后，让 `fix-validator` 执行一次 `final_batch`：核对 Patch 账本与累计 Diff，并重新执行 Security Review、Build、Test 和 Regression Review。后续 Patch 破坏先前安全不变量或安全回归测试时，先前候选结果必须失效。
...
- 只凭标题、CWE、行号变化、告警位置变化或补丁表面模式不得宣布修复成功。最终接受必须依赖 `Security Review=PASS`、`Build=PASS`、`Tests=PASS`、`Regression Review=PASS` 且 `security_regression_coverage=COVERED`。
...
`RESOLVED_BY_PRIOR_PATCH` 是 Finding 的解决归因，不是最终状态；必须同时记录 `patch_owner`，并由代码因果证据、安全回归测试和 `final-judge` 决定其最终状态。
