...
import { prepareSpawnCommand, terminateProcessTree } from "../../process/spawn.ts"
import type {
  ScanRequest,
  ScanResult,
  ScannerAdapter,
...
export const commandScannerAdapter: ScannerAdapter = {
  ...
  async scan(
    ...
    if (spec.command.some(value => typeof value !== "string") || !String(spec.command[0]).trim()) {
      return {
        status: "FAIL",
        scanner: config.name ?? "command",
        adapter: "command",
        reason: `${request.mode} 扫描命令必须是非空字符串 argv 数组`,
      }
    }
    ...
    if (!Number.isFinite(configuredTimeout) || configuredTimeout <= 0) {
      return {
        ...
        scanner: config.name ?? "command",
        adapter: "command",
        reason: `非法扫描超时时间：${String(spec.timeoutMs)}`,
      }
    }
    const timeout = Math.max(1000, Math.min(configuredTimeout, 1800000))
    let extension: string
    try {
      extension = normalizeExtension(spec.outputExtension ?? config.outputExtension)
    } catch (error) {
      return {
        status: "FAIL",
        scanner: config.name ?? "command",
    ...
    const output = path.join(outDir, `${request.mode}-${Date.now()}-${randomUUID()}${extension}`)
    const vars = {
    ...
    try {
      ...
      return {
        ...
        adapter: "command",
        command: cmd,
        reason: `扫描命令无法执行：${error instanceof Error ? error.message : String(error)}`,
      }
    } finally {
