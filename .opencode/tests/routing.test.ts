...
test("缺少证据和存在未知证据使用不同状态", () => {
  assert.equal(routeFinding({ analysis_verdict: "VULNERABLE", language: "Java" }).status, "UNCLASSIFIED")
...
test("路由时规范化常见语言别名", () => {
  assert.equal(
    routeFinding({ analysis_verdict: "VULNERABLE", raw_type: "SQL_INJECTION", language: "Node.js" }).status,
    "MATCHED",
  )
  assert.equal(
    routeFinding({ analysis_verdict: "VULNERABLE", raw_type: "SQL_INJECTION", language: "Rust" }).status,
    "NOT_SUPPORTED",
  )
})
